 ...
 * opponent's previous move, as it is likely to be repeated. Since there will be
 * 2 moves that can win the player's previous move in game of rock paper
 * scissors lizard spock, one of them will be chosen randomly. Then
...
public class AIRSPLSPGameLauncher {
	...
	public static void main(String[] args) {
		 ...
		 */
		AIRSPLSPInstructor currentInstructor = new AIRSPLSPInstructor();
		AIRSPLSPThrower currentThrower = new AIRSPLSPThrower();
		ScoreRubric currentRubric = new ScoreRubric();
		ScoreBoard currentScoreBoard;
		currentInstructor.declareRule(); // rule for the game is declared for the player.
		currentScoreBoard = new RSPLSPScoreBoard(5);

		for (int i = 1; i < 101; i++) {
			System.out.println("Round " + i);
			int tempUserMove = currentThrower.getPlayerMove();// get human player's move
			int tempComputerMove = currentThrower.getRandomComputerMove(5);// get computer's move
			AIRSPLSPThrower.determineWinner(tempComputerMove, tempUserMove, currentRubric.rubric);// determine winner.
			while (currentThrower.isTie == true || currentThrower.isBadInput == true) {
				...
				AIRSPLSPThrower.determineWinner(rematchComputerMove, rematchUserMove, currentRubric.rubric);
			}
		}
