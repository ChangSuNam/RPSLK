 ...
 * @UNI cn2521
 */
public class RSPLSPScoreBoard implements ScoreBoard {
	 ...
	 */
	public RSPLSPScoreBoard(int possibleMoves) {
	...
	public void updateTotalScore() {
		...
		for (int i = 1; i < RSPLSPScoreBoard.scoreMatrix.length; i++) {
			int currentSum = 0;
			for (int j = 1; j < RSPLSPScoreBoard.scoreMatrix[0].length; j++) {
				int currentValue = Integer.parseInt(RSPLSPScoreBoard.scoreMatrix[i][j]);
				currentSum += currentValue;

				if (j == RSPLSPScoreBoard.scoreMatrix[0].length - 1) {
					String updatedString = String.valueOf(currentSum);
					RSPLSPScoreBoard.scoreMatrix[i][j] = updatedString;
				}
			}
		}
		...
		for (int i = 1; i < RSPLSPScoreBoard.scoreMatrix.length; i++) {
			int currentSum = 0;
			for (int j = 1; j < RSPLSPScoreBoard.scoreMatrix[0].length; j++) {
				int currentValue = Integer.parseInt(RSPLSPScoreBoard.scoreMatrix[j][i]);
				currentSum += currentValue;

				if (j == RSPLSPScoreBoard.scoreMatrix[0].length - 1) {
					String updatedString = String.valueOf(currentSum);
					RSPLSPScoreBoard.scoreMatrix[j][i] = updatedString;
				}
			}
		}

		RSPLSPScoreBoard.scoreMatrix[scoreMatrix.length - 1][scoreMatrix.length - 1] += "/100";
